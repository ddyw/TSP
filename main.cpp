...
int NcMax = 2000;
//信息启发因子，期望启发式因子，全局信息素挥发参数，局部信息素挥发参数, 状态转移公式中的q0
double alpha = 2, beta = 4, rou = 0.7, alpha1 = 0.1,  qzero = 0.01;
//-----------问题二结束------------------------------------------------------------------------
...
int ACSAnt::Choose()
{
    ...
    if (q <= qzero)
    {
        ...
        if(Mad>200&&Mad<600)
        {
            ...
            double  mm = rand()/(double )RAND_MAX;
            if(m<0.2)nextCity = secondmax;
            else if(m>0.2&&m<0.3&&thirdmax!=-1)nextCity = thirdmax;
        }
        else if(Mad>600)
        {
            double m = rand()/(double)RAND_MAX;
            double  mm = rand()/(double )RAND_MAX;
             if(m<0.1)nextCity = secondmax;
            else if(m>0.1&&m<1.5&&thirdmax!=-1)nextCity = thirdmax;
            else if(m>1.5&&m<0.2)nextCity = mm*N;
        }
    }
    else
    {
        ...
        //if(p>0.7&&p<0.9&&nextCity<N-1)nextCity++;
    }
    return nextCity;
}
...
//        cout<<endl;
        cout << "Mad" << Mad << endl;
        if(globalBestLength == glbefore)
            {Mad++;}
        else {Mad = 0;}
        cout << globalBestLength << endl;
        glbefore = globalBestLength;
    }
    //输出全局最优路径
